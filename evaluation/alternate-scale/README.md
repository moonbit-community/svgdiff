...
This evaluation rerenders focused SVG pairs through `Milky2018/svg@0.3.1` at integer output scales 1, 2, 4, and 8. It records premultiplied-RGBA8 response curves and compares positive-versus-negative micro-movements in the separate `svgdiff-alternate-scale-renderer-qa/1` artifact. The checked-in measurements record the current response at each scale without entering canonical report semantics.

Run `sh scripts/test-alternate-scale-qa.sh` to reproduce [`baseline.v1.json`](baseline.v1.json). Exact-equivalent inputs are required to remain exact at every scale; other curves are observations whose drift requires review. The baseline is bound to `svgdiff-renderer-conformance-profile/27` and hashes every fixture source.
