...
Run `sh scripts/test-alternate-scale-qa.sh` to reproduce [`baseline.v1.json`](baseline.v1.json). Exact-equivalent inputs are required to remain exact at every scale; other curves are observations whose drift requires review. The baseline is bound to `svgdiff-renderer-conformance-profile/25` and hashes every fixture source.
