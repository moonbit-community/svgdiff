...
This evaluation rerenders focused SVG pairs through `Milky2018/svg@0.3.0` at integer output scales 1, 2, 4, and 8. It records premultiplied-RGBA8 response curves and compares positive-versus-negative micro-movements in the separate `svgdiff-alternate-scale-renderer-qa/1` artifact. Under `0.3.0`, both micro-movements produce zero changed pixels at every tested scale, so the directional checks are symmetric but visually unresponsive.

Run `sh scripts/test-alternate-scale-qa.sh` to reproduce [`baseline.v1.json`](baseline.v1.json). Exact-equivalent inputs are required to remain exact at every scale; other curves are observations whose drift requires review. The baseline is bound to `svgdiff-renderer-conformance-profile/26` and hashes every fixture source.
