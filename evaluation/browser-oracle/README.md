...
Run `sh scripts/test-browser-oracle.sh` to capture the fixture set twice and require byte-identical PNGs and reports. The manifest includes the complete-analysis corpus pairs plus focused alpha, clipping, and compositing conformance fixtures. Category and coverage-claim metadata feed the separate [renderer conformance comparison](../renderer-conformance/README.md); exploratory fixtures do not expand production coverage.
