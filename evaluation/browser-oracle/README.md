...
The output contains one PNG per manifest fixture plus `oracle-report.json` with source and PNG SHA-256 values, dimensions, browser identity, CLI version, and DPR. The output directory must be empty so stale artifacts cannot be mistaken for current evidence. The current 88-fixture set includes canonical pairs for HSL syntax, multiplied color/paint alpha, inherited paint opacity, and clamped opacity, the supported overlapping-child group-opacity oracle, plus six focused linear/radial gradient fixtures spanning units, transforms, templates, and multiple stops.
