...
| Wrong alignment | Unlabelled rectangles with different local geometry swap source order | Correspondences cross source indices by normalized geometry and the report remains complete with no differences. |
| Transform-blind wrong alignment | Locally identical unlabelled rectangles distinguished only by cumulative transform swap source order | Correspondences cross source indices by final device-space geometry and the report remains complete with no differences. |
...
| Resource dependency cycle | An image resource refers directly to itself | The unified graph fails admission with the same source-located cycle contract before unsupported image semantics or renderer parsing can hide the cycle. |
