...
This suite contains one focused pair for each current adversarial failure mode: false complete, false equality, source-order alignment, attribution leakage, same-domain magnitude misordering, and unsafe local-reference expansion. The cases are small counterexamples with executable invariants, not a claim of general adversarial robustness.
