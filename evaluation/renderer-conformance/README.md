...
Conformance profile: `svgdiff-renderer-conformance-profile/29`

This evaluation compares raw `Milky2018/svg@0.5.2` with the Chromium oracle. Its report shape remains `svgdiff-renderer-conformance/1`, independently from the conformance claim identified by `svgdiff-renderer-conformance-profile/29` in the production Comparison Profile. Engine regressions separately prove the project-owned normalizers, compositors, and guards because this adapter deliberately isolates the underlying dependency.
...
Fixtures are classified as geometry, paint, alpha, clipping, or compositing. `supported` means the fixture exercises the current complete-eligible contract and `guarded` identifies a measured input that has a production Diagnostic. The 141-case `Milky2018/svg@0.5.2` baseline contains 98 exact and 43 divergent observations against Chromium 151. Profile `/29` retains the `/28` classifications and dispositions while making cross-platform browser evidence explicit. Chromium on `macos-arm64` and `linux-x64` produces different exact measurements for two pattern fixtures and the admitted blend grid; [`platform-variants.v1.json`](platform-variants.v1.json) records only those three measured tuples. All other fields and 138 fixtures remain byte-exact across the two admitted hosts. Viewport-relative lengths, remaining gradients and patterns, embedded PNG/JPEG composition, fractional alpha, polygon clip rules, curved/point/stroke/marker rasterization, and skew remain divergent. Both images are normalized to premultiplied RGBA8 before measuring changed pixels, changed-pixel fraction, RMSE, and maximum channel delta. Declared browser source/canonical pairs normally require byte equality; the blend-grid companion alone permits a maximum per-channel rounding delta of two because the exact W3C/product-compositor value differs by at most one on `macos-arm64` Chromium and two on `linux-x64` Chromium.

Run `sh scripts/test-renderer-conformance.sh` to capture Chromium output, render the same sources through the pinned renderer, and compare the result with `baseline.v1.json` plus the exact host-specific measurements in `platform-variants.v1.json`. The report records the normalized host platform, browser version, DPR, CLI version, and fixture source-set hash without retaining platform-specific User-Agent text. Unknown hosts and unregistered measurement changes fail validation. A baseline divergence is an observation, not an acceptance decision. Every divergence must be handled by the following roadmap item before it may influence the coverage contract.
