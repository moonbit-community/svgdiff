...
- region_ids are the IDs in the containing event's regions. possible_cause_changed_fact_ids are the union of explicit possible_causes.candidate_difference_ids for event_region scope; comparison scope means every difference_groups item ID. The output field name is retained for benchmark compatibility. Use sound_overapproximation only when every cited region has that guarantee, not_established for any other nonempty region set, and not_applicable when there are no regions. Include possible_causes limitation IDs.
