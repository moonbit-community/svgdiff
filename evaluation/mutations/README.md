...
The coverage contract names all six current basic subject kinds and forty current authored properties across fifty-three cases. Sixteen cases remain complete and produce thirty-two complete directional comparisons; active stroke, marker, curved/point geometry, fractional opacity, gradient raster, and pattern raster cases remain partial under their documented guards. The complete paint cases include an inline `!important` mutation whose later normal declaration must not win, a class-matched stylesheet mutation, an inherited rectangle fill mutation with owner-level provenance and complete leaf evidence, equivalent CSS-wide and CSS Color 3 fill rewrites, a changed `currentColor` dependency, a changed custom-property dependency, and a missing-server URL whose selected fallback changes. Fill and stroke opacity are independently mutated as continuous numeric properties. Gradient cases independently mutate `gradientUnits`, `gradientTransform`, and a radial stop color; pattern cases independently mutate `patternUnits`, `patternTransform`, and child fill; each must produce the declared resource fact, identify the affected fill consumer, and retain the renderer guard without weakening source/computed causality. Stroke cases cover paint alpha, width, cap, join, miter limit, dash array, dash offset, vector effect, and percentage lengths. Marker cases cover host attachment plus resource units, viewport width, reference point, orientation, and percentage lengths while proving that every authored fact remains reportable and linked to the affected host. Absolute and percentage geometry mutations verify that authored units remain distinct while canonical used values and magnitudes drive comparison. The viewport case verifies that one changed root declaration reaches its affected leaf and every complete region's Cause Envelope.
