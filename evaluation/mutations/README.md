...
The coverage contract names all six current basic subject kinds and thirty-one current authored properties across thirty-seven cases. Eight cases remain complete and produce sixteen complete directional comparisons; active stroke, marker, curved/point geometry, fractional opacity, and inherited-value cases are intentionally partial under their documented guards. Stroke cases cover width, cap, join, miter limit, dash array, dash offset, vector effect, and percentage lengths. Marker cases cover host attachment plus resource units, viewport width, reference point, orientation, and percentage lengths while proving that every authored fact remains reportable and linked to the affected host. Absolute and percentage geometry mutations verify that authored units remain distinct while canonical used values and magnitudes drive comparison. The viewport case verifies that one changed root declaration reaches its affected leaf and every complete region's Cause Envelope.
