...
The coverage contract names all six current basic subject kinds and thirty-three current authored properties across forty-three cases. Fourteen cases remain complete and produce twenty-eight complete directional comparisons; active stroke, marker, curved/point geometry, and fractional opacity cases remain partial under their documented guards. The complete paint cases include an inline `!important` mutation whose later normal declaration must not win, a class-matched stylesheet mutation, an inherited rectangle fill mutation with owner-level provenance and complete leaf evidence, an equivalent CSS-wide fill rewrite, a changed `currentColor` dependency, and a changed custom-property dependency. Stroke cases cover width, cap, join, miter limit, dash array, dash offset, vector effect, and percentage lengths. Marker cases cover host attachment plus resource units, viewport width, reference point, orientation, and percentage lengths while proving that every authored fact remains reportable and linked to the affected host. Absolute and percentage geometry mutations verify that authored units remain distinct while canonical used values and magnitudes drive comparison. The viewport case verifies that one changed root declaration reaches its affected leaf and every complete region's Cause Envelope.
