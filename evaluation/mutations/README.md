...
The coverage contract names all six current basic subject kinds and forty-four current authored properties across fifty-nine cases. Twenty cases remain complete and produce forty complete directional comparisons with forty-two complete regions; active stroke, marker, curved/point geometry, fractional opacity, gradient raster, pattern raster, and clip-path cases remain partial under their documented guards. The complete paint cases include an inline `!important` mutation whose later normal declaration must not win, a class-matched stylesheet mutation, an inherited rectangle fill mutation with owner-level provenance and complete leaf evidence, equivalent CSS-wide and CSS Color 3 fill rewrites, a changed `currentColor` dependency, a changed custom-property dependency, a missing-server URL whose selected fallback changes, and an inactive ordinary `clip-rule` mutation. Active paint-order and self-intersecting fill-rule mutations remain source/computed-reportable behind their existing stroke or point-shape raster guards. Fill and stroke opacity are independently mutated as continuous numeric properties. Gradient cases independently mutate `gradientUnits`, `gradientTransform`, and a radial stop color; pattern cases independently mutate `patternUnits`, `patternTransform`, and child fill; each must produce the declared resource fact, identify the affected fill consumer, and retain the renderer guard without weakening source/computed causality. Stroke cases cover paint alpha, width, cap, join, miter limit, dash array, dash offset, vector effect, and percentage lengths. Marker cases cover host attachment plus resource units, viewport width, reference point, orientation, and percentage lengths while proving that every authored fact remains reportable and linked to the affected host. Absolute and percentage geometry mutations verify that authored units remain distinct while canonical used values and magnitudes drive comparison. The viewport case verifies that one changed root declaration reaches its affected leaf and every complete region's Cause Envelope. Two use cases verify definition-owned fill fan-out and placement-owned `x` geometry through deterministic instance identity. The active stacking-order mutation independently declares the inverted structural relationship and requires it in both direction-correct complete Cause Envelopes.
...
The aggregate [M2 soundness gate](../m2-soundness-gate/README.md) uses this suite as its supported-property causal oracle alongside focused alignment/region tests, curated annotations, adversarial cases, and report-reference closure.
