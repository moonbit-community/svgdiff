...
The coverage contract names all six current basic subject kinds and thirty-one current authored properties across thirty-nine cases. Ten cases remain complete and produce twenty complete directional comparisons; active stroke, marker, curved/point geometry, fractional opacity, and inherited-value cases are intentionally partial under their documented guards. The complete paint cases include an inline `!important` mutation whose later normal declaration must not win and a class-matched stylesheet mutation. Stroke cases cover width, cap, join, miter limit, dash array, dash offset, vector effect, and percentage lengths. Marker cases cover host attachment plus resource units, viewport width, reference point, orientation, and percentage lengths while proving that every authored fact remains reportable and linked to the affected host. Absolute and percentage geometry mutations verify that authored units remain distinct while canonical used values and magnitudes drive comparison. The viewport case verifies that one changed root declaration reaches its affected leaf and every complete region's Cause Envelope.
