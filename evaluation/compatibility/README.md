...
| Optional alignment evidence absent | Accept with uncertainty evidence unreported. | Valid `1.31`. |
| Optional instance context absent | Accept with direct-versus-instance context unreported by the legacy-shaped producer. | Valid `1.31`. |
| Optional Diagnostic source locations absent | Accept with locations unreported. | Valid `1.31`. |
| Legacy optional fields absent | Accept with explicit legacy handling. | Valid `1.31`. |
| Unknown additive top-level field under declared `1.31` | Accept while ignoring the unknown field. | Valid `1.31`. |
| Previous renderer and conformance identities | Accept as a known current-shape report. | Valid `1.31`. |
| Unknown schema version | Reject before interpretation. | Invalid under every released Schema. |
| Unknown renderer identity | Reject before rendered interpretation. | Structurally valid `1.31`. |
| Unknown renderer conformance profile | Reject before rendered interpretation. | Structurally valid `1.31`. |
| Unknown ordering policy | Reject before ranking. | Invalid `1.31`. |
