...
The current policy accepts schemas `1.0` through `1.33`, legacy ordering policy `v1_domain_lexicographic`, current policy `v2_domain_lexicographic`, the raw historical `mizchi/svg@0.2.1` renderer identity, all fifteen production renderer identities, and conformance profiles `/1` through `/25`. Current producers emit `1.33` with bounded static selector matching, the author cascade seam, ordinary inheritance, computed CSS values, deterministic solid colors, paint opacity, isolated static container opacity, deterministic rectangular clipping, static alpha/luminance masking, a bounded static `feOffset` filter graph, opaque binary-alpha CSS blending and isolation, conservative effect bounds, SVG 2 paint URL fallback selection, inherited paint order and winding rules, complete static linear/radial gradient semantics, admitted static pattern semantics, authored-unit used values, marker placement and viewport semantics, canonical stroke and basic-shape geometry, typed transform effects, deterministic same-document use-instance identity, source-structural alignment for groups, text, use hosts, and visual resource definitions, transform- and conservative-painted-bounds-aware rendered-subject correspondence, consequence-aware structural relationships, a private typed cross-resource dependency graph, bounded embedded or explicitly bundled PNG/JPEG resource evidence, nonvisual descriptive-content exclusion, and v2 transform-specific tuples. Explicit legacy migration cases restore the renderer, conformance, and alignment evidence identities emitted with each old schema and remove fields introduced after each identity, proving that the archived Schemas remain readable. The policy also accepts previous renderer identities under the current shape, optional-field omissions with explicit legacy handling, and an unknown additive top-level field under a recognized identity. It rejects unknown schema, renderer, and renderer-conformance identities before interpreting their evidence and rejects unknown ordering policies before using their component vectors. The current Schema treats renderer identities as non-empty opaque strings; acceptance belongs to this policy rather than the report-shape contract.

The registry retains Schemas `1.0` through `1.32` as legacy and names Schema `1.33` as current. Each entry names its checked-in Schema, accepted ordering policy, canonical-example manifest, and migration cases. The shared project validator audits the complete assertion vocabulary used by every Schema and rejects unknown future keywords. It is not advertised as a general-purpose JSON Schema implementation.

No schema `2.0` or ordering policy v3 migration is implemented; those values are deliberately unknown test inputs. An unknown declared Schema must fail every released Schema, and an unknown ordering policy under `1.33` must fail the current policy constraint before ranking.

## Cases

| Case | Consumer decision | Schema validation |
| --- | --- | --- |
| Current schema and policy | Accept as current. | Valid `1.33`. |
...
| Optional alignment evidence absent | Accept with uncertainty evidence unreported. | Valid `1.33`. |
| Optional instance context absent | Accept with direct-versus-instance context unreported by the legacy-shaped producer. | Valid `1.33`. |
| Optional Diagnostic source locations absent | Accept with locations unreported. | Valid `1.33`. |
| Legacy optional fields absent | Accept with explicit legacy handling. | Valid `1.33`. |
| Unknown additive top-level field under declared `1.33` | Accept while ignoring the unknown field. | Valid `1.33`. |
| Previous renderer and conformance identities | Accept as a known current-shape report. | Valid `1.33`. |
| Unknown schema version | Reject before interpretation. | Invalid under every released Schema. |
| Unknown renderer identity | Reject before rendered interpretation. | Structurally valid `1.33`. |
| Unknown renderer conformance profile | Reject before rendered interpretation. | Structurally valid `1.33`. |
| Unknown ordering policy | Reject before ranking. | Invalid `1.33`. |
