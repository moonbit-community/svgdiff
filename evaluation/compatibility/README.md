...
| Optional alignment evidence absent | Accept with uncertainty evidence unreported. | Valid `1.7`. |
| Optional Diagnostic source locations absent | Accept with locations unreported. | Valid `1.7`. |
| Legacy optional fields absent | Accept with explicit legacy handling. | Valid `1.7`. |
| Unknown additive top-level field under declared `1.7` | Accept while ignoring the unknown field. | Valid `1.7`. |
| Previous renderer and conformance identities | Accept as a known current-shape report. | Valid `1.7`. |
| Unknown schema version | Reject before interpretation. | Invalid under every released Schema. |
| Unknown renderer identity | Reject before rendered interpretation. | Structurally valid `1.7`. |
| Unknown renderer conformance profile | Reject before rendered interpretation. | Structurally valid `1.7`. |
| Unknown ordering policy | Reject before ranking. | Invalid `1.7`. |
