...
| 2026-07-17 | Terminal multidimensional magnitude gate | Validated nine named raw evidence families, exact unavailable states, canonical examples, production probes, mutation controls, Agent scoring, and all 354 retained model magnitude claims without creating a universal score. |
| 2026-07-17 | Terminal text-only Agent reliability gate | Bound the retained thirteen-case model observation to independent change, main-event, localization, possible-cause, coverage, traceability, safety, and report-soundness obligations. |
| 2026-07-17 | Terminal operational readiness gate | Composed deterministic output, installation, hostile-input boundaries, versioned contracts, and the fixed supported-native matrix while preserving explicit sandbox and reproducible-build non-guarantees. |
| 2026-07-17 | Post-terminal deferred blocker review | Stopped continuous execution with 21 unchecked items grouped below; no group is implementation-ready without its recorded new evidence, authorization, or external capability. |
...
- [x] Every adopted dynamic, `foreignObject`, wide-gamut, platform, or cross-renderer capability has a pinned environment and separate profile identity.
...
- [x] The tool remains deterministic, installable, secure on hostile input, versioned, and reproducible across supported environments.

## Deferred blocker register

Status snapshot: 2026-07-17. Terminal acceptance is complete, while the 21 unchecked items above remain intentionally deferred. This table records why continuous checklist execution stopped; it does not check, schedule, reject, or supersede any capability. Resume one group only by opening a separate `ISS-###` item after its reopen evidence exists.

| Blocked group | Unchecked items | Current blocker | Required reopen evidence |
| --- | ---: | --- | --- |
| Cross-subject Visual Event aggregation | 1 | The accepted report-only Agent benchmark shows no fragmentation failure, so aggregation would add identity, magnitude, region, and causal complexity without an observed task need. | A minimized Agent failure plus labeled cases and a deterministic proposal for membership, ordering, traceability, magnitude, regions, causes, ambiguity, and compatibility. |
| Calibrated Impact and severity | 3 | Calibration v1 rejected every production candidate: no independent reviewer agreement, no ranked multi-event case, weak tier coverage, one target without policy inputs, and unstable held-out results. This also blocks the remaining M3 gate. | Independent review, ranked multi-event labels, broader balanced tiers, complete policy inputs, profile-sensitivity evidence, and a newly versioned policy that passes held-out gates. |
| Exact contribution weights | 1 | Complete Cause Envelopes and the accepted Agent observation currently solve the possible-cause task without a precision failure or quantitative-weight requirement. | A minimized Agent ambiguity after safe pruning, a failed versioned precision threshold, a concrete product requirement for weights, or a stable dependency contribution API with acceptable ownership cost. |
| Calibrated alignment confidence | 1 | Current alignments expose deterministic evidence and ambiguity but there is no probability calibration corpus. | Independent correspondence ground truth, representative ambiguous cases, profile sensitivity, calibration-error targets, and explicit abstention behavior. |
| Deterministic fonts and M4 | 10 | Font Bundle and runtime decisions exist, but font implementation remains explicitly unscheduled; no font runtime module, matcher, shaper integration, SVG text layout, glyph evidence, or multilingual benchmark is implemented. | Explicit authorization to resume font work, then every Font Runtime safety/conformance gate, deterministic matching/shaping/layout/report integration, cross-target glyph evidence, and multilingual Agent evaluation. |
| Cross-browser and platform profiles | 2 | The advanced adopted-profile set is empty and no concrete Agent task requires cross-target stability evidence. | A concrete task, at least two useful compatible cells, closed resource/environment identities, versioned mapping and normalization controls, hard limits, and scoreable missing/disagreement semantics. |
| Animation comparison | 1 | Only the timeline/checkpoint contract is accepted; no closed timing engine, profile Schema, conformance corpus, or Agent evidence exists. | A concrete Agent task plus the complete profile Schema, deliberately small project-owned mechanism slice, edge-case fixtures, hard limits, conformance evidence, and temporal Agent evaluation. |
| `foreignObject` rendering | 1 | General support requires a deterministic host-language layout engine and depends on unfinished font, resource, layout, paint, and integration work. | A concrete Agent task, pinned closed host-language slice and engine, completed dependent profiles, hostile-input/process limits, conservative provenance, cross-target conformance, and Agent evaluation. |
| Network-backed acquisition | 1 | Comparison-time I/O remains a permanent non-goal and no separately accepted secure prefetch implementation exists. | A concrete task, versioned snapshot/prefetch schemas, pinned URL evaluator, recursive closure, hard limits, SSRF/credential/path protections, security and license review, deterministic replay, and a separately accepted isolated implementation plan. |
