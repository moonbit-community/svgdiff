...
```sh
sh scripts/check-release-tag.sh v0.5.28
git tag -a v0.5.28 -m "svgdiff v0.5.28"
git push origin v0.5.28
```

Replace `0.5.28` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
