...
```sh
sh scripts/check-release-tag.sh v0.4.14
git tag -a v0.4.14 -m "svgdiff v0.4.14"
git push origin v0.4.14
```

Replace `0.4.14` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
