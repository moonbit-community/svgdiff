...
```sh
sh scripts/check-release-tag.sh v0.4.13
git tag -a v0.4.13 -m "svgdiff v0.4.13"
git push origin v0.4.13
```

Replace `0.4.13` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
