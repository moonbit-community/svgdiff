...
```sh
sh scripts/check-release-tag.sh v0.4.9
git tag -a v0.4.9 -m "svgdiff v0.4.9"
git push origin v0.4.9
```

Replace `0.4.9` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
