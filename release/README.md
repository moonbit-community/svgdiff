...
```sh
sh scripts/check-release-tag.sh v0.4.3
git tag -a v0.4.3 -m "svgdiff v0.4.3"
git push origin v0.4.3
```

Replace `0.4.3` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
