...
```sh
sh scripts/check-release-tag.sh v0.7.0
git tag -a v0.7.0 -m "svgdiff v0.7.0"
git push origin v0.7.0
```

Replace `0.7.0` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
