...
```sh
sh scripts/check-release-tag.sh v0.4.1
git tag -a v0.4.1 -m "svgdiff v0.4.1"
git push origin v0.4.1
```

Replace `0.4.1` with the current `moon.mod` version. The workflow rejects any mismatch before packaging and uses `--verify-tag` before publication.
