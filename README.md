...
Admitted isolated entity geometry also records a versioned symmetric nearest-boundary distribution over pinned-renderer alpha-support pixels: before/after sample counts plus mean, nearest-rank p95, and maximum CSS-pixel distance. This quantized outcome can be zero while exact geometry changes, and remains null when isolation or a two-sided boundary is unavailable; it is not soft coverage, color difference, or an importance score.
