...
```sh
moon add Milky2018/svgdiff@0.5.27
```
...
`impact_assessment.frontier_groups` exposes every main Visual Event under the current uncalibrated Pareto policy. Follow its event and Atomic Difference links; exact ties and incomparable tradeoffs are intentional and the serialized group order is not a severity ranking.
