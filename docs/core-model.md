...
Status: current model for Structured Report schema `1.33`
...
Schema `1.32` adds a separate source-structural alignment inventory for `g`, `text`, `use`, `symbol`, `linearGradient`, `radialGradient`, `pattern`, `marker`, `clipPath`, `mask`, and `filter`. Compatible kinds pair by authored-ID hint, structural path, then stable kind-local source order, with insertion and deletion for remaining endpoints. This inventory does not pass containers or resources through shape geometry. Text correspondence does not imply font, shaping, glyph, or rendered equality, and resource Atomic Differences remain unattached until Visual Resources receive their own alignment contract. Schema `1.33` changes rendered-leaf correspondence as described above; stronger repeated-subject matching, broader many-to-many validation, and separate Visual Resource alignment remain roadmap work.
...
Unavailable components are `null`, not numeric zero. Intrinsic raster metrics never populate final-canvas raster fields. When intrinsic dimensions differ, the dimensions remain present and per-pixel metrics are null because schema `1.33` declares no implicit resampling policy. Insertion and deletion additionally use `PresenceMagnitude` to record subject count, geometric bounds, painted area, and viewport fractions from the side on which the content exists where that evidence is available.
...
A `VisualEvent` is the primary agent-facing grouping unit. In schema `1.33` it records one primary subject ID, referenced Atomic Difference IDs, one rendered outcome, and zero or more Difference Regions.
...
The schema `1.33` top-level object contains exactly these conceptual sections:
...
## Not implemented in schema 1.33
