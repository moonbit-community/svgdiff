...
Status: current model for Structured Report schema `1.13`
...
- `renderer_id`, currently fixed by the producer to `svgdiff/style-precedence-normalizer@3+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1`.
- `renderer_conformance_profile_id`, currently fixed by the producer to `svgdiff-renderer-conformance-profile/10`.
...
A `VisualEvent` is the primary agent-facing grouping unit. In schema `1.13` it records one primary subject ID, referenced Atomic Difference IDs, one rendered outcome, and zero or more Difference Regions.
...
The schema `1.13` top-level object contains exactly these conceptual sections:
...
## Not implemented in schema 1.13
