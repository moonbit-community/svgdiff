...
Caller-supplied local raster bundles are now implemented under the separate [local resource bundle policy](local-resource-bundle-policy.md). Full missing, cyclic, invalid, and unused cross-resource behavior beyond the minimum image-bundle conditions follows after that. Network-backed acquisition, if ever supported, must be an explicit prefetch step outside comparison. Embedded SVG and final image compositing remain deferred until their complete deterministic execution and conformance profiles are accepted.
