...
The current public `ResourceBundleEntry` is exactly `{ locator, media_type, bytes }`. The engine trims and case-sensitively matches the locator, accepts only PNG/JPEG media types, keeps sides separate, validates fixed budgets, and performs no I/O ([implementation](../../engine/resource_bundle.mbt), [implemented policy](local-resource-bundle-policy.md)). It does not resolve a base, normalize URLs, follow redirects, or model requests. Referenced content uses the bounded raster path; unused payload is not decoded or compared. Missing entries are partial, invalid global configuration and budget overflow fail admission, and unused valid entries make no report difference ([Resource Outcome Policy](../resource-outcome-policy.md)).
