...
The later accepted [general Resource Snapshot boundary](../general-resource-snapshot-bundles.md) preserves this exact opaque matching contract and introduces parallel versioned URL/request/response artifacts for future work; it is not a migration or reinterpretation of the current `ResourceBundle` API.
