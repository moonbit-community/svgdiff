...
Status: internal typed-model policy; not serialized by schema `3.0`

Last verified: 2026-07-17

The engine retains a typed `ImpactAssessment` for internal experiments and legacy library callers. Schema `3.0` deliberately does not serialize it. The assessment is not a severity label, universal similarity score, visibility judgment, equality rule, or total cross-domain ranking.
