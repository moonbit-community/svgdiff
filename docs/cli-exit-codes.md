...
| `1` | Analysis produced a failed report, or an explicit checkpoint budget interrupted comparison. | Yes for failed analysis; no for interruption |
...
| `--max-checkpoints` exhausted | `1` | Read stderr; no report was produced and no evidence conclusion is valid. |
...
| JSON, HTML, or summary output write failure | `2` | Read stderr; treat any requested output artifact as unavailable. |
