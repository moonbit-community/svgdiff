...
The production source-input seam materializes the first graph edge as a private subject-influence token index. Every Changed Fact token is indexed under each analyzer-owned `affected_subject_id`. Entity events query both sides of their entity-role Subject Alignment, using rendered instance identity for `use`; resource and relationship events query the affected subjects reached by their direct facts. Direct tokens remain present even when no subject edge exists. Supported group, clip, mask, filter, blend, isolation, and stacking events additionally union their analyzer-owned operation participants before querying the same index; every event sharing an exact Difference Region contributes its tokens. This covers admitted geometry, paint, resource, transform, viewport, use-instance, structural, effect, and compositing inputs. It is region-level rather than tile-level provenance.
...
The production localization seam preserves those independently optional side bounds through ordinary geometry, images, markers, group opacity, clips, masks, and filters. It unions them only when selecting or emitting an event's Difference Regions; this is a conservative spatial envelope and does not assign pixels or causes to one side. For admitted leaf geometry, paint, and presence Events, a separate isolated before/after subject rendering filters the final canvas mask and can prove an empty Event. The resulting region remains conservative because coincident isolated and final pixels do not establish contribution through occlusion or compositing. Without isolation evidence, the bounds-filtered canvas candidate is wider but equally explicit about that limitation.
...
The [M2 soundness gate](../evaluation/m2-soundness-gate/README.md) composes these causal properties with alignment cardinality, side-typed localization, curated annotations, adversarial cases, and report-reference closure. Its claim remains conditional on the declared supported profile and permits conservative false positives.
