...
The deterministic mutation suite adds a complementary containment property across all six supported basic subject kinds and thirty-three authored properties. Each of the forty-three generated pairs changes exactly one independently declared fact and is evaluated in both comparison directions. Every one of the thirty regions in twenty-eight complete directional comparisons must contain the direction-correct fact under `sound_overapproximation`; guarded active-stroke, curved/point geometry, and fractional-opacity cases must not claim that guarantee. The complete inherited-rectangle, CSS-wide fill, `currentColor`, and custom-property mutations prove ancestor ownership, computed dependency, and leaf consequence containment, while the inherited-circle case remains partial only because of its independent curved-raster guard. This exercises occupied and vacated outcomes plus stroke, transform, viewport, cascade, inheritance, and computed CSS changes without pretending that future exact transformed outlines, filters, masks, or compositing provenance are already implemented. A negative control removes the fact from one envelope and must fail.
