...
The deterministic mutation suite adds a complementary containment property across all six supported basic subject kinds and nineteen authored properties. Each generated pair changes exactly one independently declared fact and is evaluated in both comparison directions. Every region in all thirty-eight complete directional comparisons must contain the direction-correct fact under `sound_overapproximation`; the two partial cases in both directions must not claim the guarantee. This exercises occupied and vacated outcomes plus a transform change without pretending that future precise transformed localization, filter, mask, or compositing provenance is already implemented. A negative control removes the fact from one envelope and must fail.
