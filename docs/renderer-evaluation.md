...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 60-case dependency baseline records 25 exact cases and 35 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, markers, and authored units. Four unit-bearing inputs have exact Chromium canonical companions and executable production-normalizer dispositions; unit-bearing stroke and marker fixtures retain their existing raster guards. Integer translation, scale, matrix, quadrant rotation, the four root/nested viewport fixtures, zero-radius circle geometry, and fill-only line behavior remain exact; skew, curved-shape boundaries, filled point-shape boundaries, active strokes, and markers remain guarded. The exploratory clipping case is exact. Baseline observations, accepted dispositions, production guards, and all four accepted normalizers are bound by `svgdiff-renderer-conformance-profile/8`, independently from report schema `1.11` and the production renderer identity.
