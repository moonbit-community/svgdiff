...
### Cascade and ordinary inheritance

The pinned `mizchi/svg@0.2.1` dependency applies conflicting presentation attributes and inline declarations in XML attribute order, does not honor inline importance, and inherits only a subset of the admitted visual-property surface. Production therefore uses `svgdiff/style-precedence-normalizer@3+ordinary-inheritance-normalizer@1+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1`: private renderer-input adapters materialize cascade winners and complete admitted ordinary inherited values before canonicalizing length, stroke, and basic-shape used values. The original source remains untouched for source-level evidence. Invalid or unsupported syntax remains unchanged and emits the corresponding limitation Diagnostic. Upstream PR [`mizchi/svg#4`](https://github.com/mizchi/svg/pull/4) remains useful but is no longer a product blocker.

The Chromium inherited-stylesheet fixture is byte-equivalent to its leaf-local canonical companion. Raw `mizchi/svg` diverges because it does not apply the stylesheet declaration; the production comparison must prove computed equivalence and zero changed pixels through the cascade and ordinary-inheritance normalizers. Inherited active-stroke and marker inputs retain their existing independent raster guards.
...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 70-case dependency baseline records 30 exact cases and 40 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, markers, authored units, cascade precedence, and ordinary inheritance. Four unit-bearing, four cascade, and one inherited-stylesheet input have exact Chromium canonical companions and executable production-normalizer dispositions; active strokes, markers, skew, and curved or point-shape boundaries retain their guards. Baseline observations, accepted dispositions, production guards, and all five normalizer modules are bound by `svgdiff-renderer-conformance-profile/11`, independently from report schema `1.14` and the production renderer identity.
