...
Paths, general affine transforms outside the admitted integer axis-transform slice, general CSS, gradient or pattern rasterization, filters, masks, clipping, blending, text layout, and external resources are not accepted as complete merely because the dependency exposes related types or rendering functions. Static linear/radial gradient and admitted static pattern source/computed semantics are project-owned; their rendered layers remain guarded. Viewport mapping is admitted only for the project-owned parser and integer-axis renderer slice established by the exact root and nested fixtures. Each remaining capability needs project-level conformance cases and explicit coverage behavior before the v1 scope expands.
...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 106-case dependency baseline records 43 exact cases and 63 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, markers, authored units, cascade precedence, inheritance, computed CSS values, solid-color syntax, paint opacity, paint fallback selection, paint order, winding rules, gradients, and patterns. Six focused gradient fixtures cover linear/radial geometry, both coordinate-unit modes, transforms, templates, and multi-stop opacity; all six diverge. Six focused pattern fixtures cover tile/content units, transforms, viewBox mapping, and templates; one is exact and five diverge. The missing-server fallback-color and no-fallback fixtures plus their canonical companions are all exact. Inherited fill-rule source and leaf forms are exact; inherited paint order becomes exact after expansion to the canonical leaf form, while active stroke rasterization remains guarded. Clip-rule and self-intersecting fill-rule cases retain clipping or point-shape guards. Four unit-bearing, four cascade, one inherited-stylesheet, one CSS-wide, one custom-property/`currentColor`, and four color/opacity inputs have Chromium canonical companions. HSL receives an executable production-normalizer disposition; fractional color/paint-alpha pairs retain their renderer guard because even the canonical companion differs by one premultiplied RGBA8 level. Baseline observations, accepted dispositions, production guards, and all seven normalizer modules are bound by `svgdiff-renderer-conformance-profile/17`, independently from report schema `1.20` and the production renderer identity.
