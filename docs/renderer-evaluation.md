...
The checked-in profile `/28` renderer-conformance baseline compares raw `Milky2018/svg@0.5.2` with Chromium 151 at DPR 1: 98 cases are exact and 43 diverge out of 141. Compared with profile `/27`, absolute-unit, percentage, nested-viewport, and percentage-stroke fixtures become exact with no regression; three still-divergent gradient/blend fixtures also have smaller errors. The fixture source set is unchanged.

Every remaining divergence has an executable disposition. Profile `/28` deliberately retains the existing conservative renderer Diagnostics where one exact fixture does not prove the full transform, pattern, or stroke capability domain. The previously established `renderer_use_inherited_paint_raster_unproven` case still diverges. These guards limit only Rendered Evidence; exact source/computed differences and conservative localization remain available.
