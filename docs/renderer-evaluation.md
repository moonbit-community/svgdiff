...
The pinned `mizchi/svg@0.2.1` dependency applies conflicting presentation attributes and inline declarations in XML attribute order. Production therefore uses `svgdiff/style-precedence-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1`: a private renderer-input adapter first resolves safe inline precedence, then canonicalizes admitted unitless stroke and basic-shape used geometry. The original source remains untouched for source-level evidence. Complete supported overlaps are order-independent; invalid or unsupported syntax remains unchanged and emits the corresponding limitation Diagnostic. Upstream PR [`mizchi/svg#4`](https://github.com/mizchi/svg/pull/4) remains useful but is no longer a product blocker.
...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 50-case dependency baseline records 21 exact cases and 29 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, and markers. Zero-width stroke is exact; active caps, joins, miter limits, dashes, dash offsets, non-scaling stroke, and all five marker fixtures diverge and receive dedicated guards. The marker cases cover attachment roles, auto-start-reverse, stroke-width units, viewport/reference mapping, and zero-size suppression; the last case shows the pinned renderer painting 40 pixels that Chromium suppresses. Integer translation, scale, matrix, quadrant rotation, the four root/nested viewport fixtures, zero-radius circle geometry, and fill-only line behavior remain exact; skew, curved-shape boundaries, and filled point-shape boundaries remain guarded. The exploratory clipping case is exact. Production adapter regressions are exercised through engine and CLI tests because the raw evaluation adapter intentionally isolates the dependency. Baseline observations, accepted dispositions, production guards, and all three accepted normalizers are bound by `svgdiff-renderer-conformance-profile/7`, independently from report schema `1.10` and the production renderer identity.
