...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 68-case dependency baseline records 29 exact cases and 39 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, markers, authored units, and cascade precedence. Four unit-bearing and four cascade inputs have exact Chromium canonical companions and executable production-normalizer dispositions; active strokes, markers, skew, and curved or point-shape boundaries retain their guards. Baseline observations, accepted dispositions, production guards, and all four normalizer modules are bound by `svgdiff-renderer-conformance-profile/10`, independently from report schema `1.13` and the production renderer identity.
