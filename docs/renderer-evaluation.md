...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 32-case dependency baseline records 20 exact cases and 12 divergences across geometry, paint, alpha, clipping, compositing, and viewport mapping. Integer translation, scale, matrix, quadrant rotation, the four root/nested viewport fixtures, zero-radius circle geometry, and fill-only line behavior are exact; both skew fixtures, curved-shape boundaries, and filled point-shape boundaries diverge and are guarded. The exploratory clipping case is exact. Production adapter regressions are exercised through engine and CLI tests because the raw evaluation adapter intentionally isolates the dependency. Baseline observations, accepted dispositions, production guards, and both accepted normalizers are bound by `svgdiff-renderer-conformance-profile/5`, independently from report schema `1.8` and the production renderer identity.
