...
Paths, general affine transforms outside the admitted integer axis-transform slice, general CSS, gradient or pattern rasterization, embedded-image final compositing, filters, masks, general clipping, blending, text layout, and external resources are not accepted as complete merely because a dependency exposes related types or rendering functions. Static linear/radial gradient, admitted static pattern, bounded PNG/JPEG resource semantics, and local rectangular clip resolution/bounds are project-owned; unavailable rendered layers remain guarded. Viewport mapping and rectangular clip rasterization are admitted only for the project-owned semantic slice established by their exact fixtures. Each remaining capability needs project-level conformance cases and explicit coverage behavior before the v1 scope expands.
