...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 25-case dependency baseline records 18 exact cases and 7 divergences across geometry, paint, alpha, clipping, compositing, and viewport mapping. Integer translation, scale, matrix, quadrant rotation, and the four root/nested viewport fixtures are exact; both skew fixtures diverge and are guarded. The exploratory clipping case is exact. Production adapter regressions are exercised through engine and CLI tests because the raw evaluation adapter intentionally isolates the dependency. Baseline observations, accepted dispositions, production guards, and the accepted style-precedence adapter are bound by `svgdiff-renderer-conformance-profile/4`, independently from report schema `1.7` and the production renderer identity.
