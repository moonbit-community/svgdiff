...
The independent [renderer conformance comparison](../evaluation/renderer-conformance/README.md) normalizes browser and raw `mizchi/svg@0.2.1` pixels to premultiplied RGBA8. Its 94-case dependency baseline records 36 exact cases and 58 divergences across geometry, paint, alpha, clipping, compositing, viewport mapping, stroke geometry, markers, authored units, cascade precedence, inheritance, computed CSS values, solid-color syntax, paint opacity, gradients, and patterns. Six focused gradient fixtures cover linear/radial geometry, both coordinate-unit modes, transforms, templates, and multi-stop opacity; all six diverge. Six focused pattern fixtures cover tile/content units, transforms, viewBox mapping, and templates; one is exact and five diverge. Four unit-bearing, four cascade, one inherited-stylesheet, one CSS-wide, one custom-property/`currentColor`, and four color/opacity inputs have Chromium canonical companions. HSL receives an executable production-normalizer disposition; fractional color/paint-alpha pairs retain their renderer guard because even the canonical companion differs by one premultiplied RGBA8 level. Baseline observations, accepted dispositions, production guards, and all seven normalizer modules are bound by `svgdiff-renderer-conformance-profile/15`, independently from report schema `1.18` and the production renderer identity.
