...
The typed engine model projects encountered renderer-specific Diagnostics into capability-gap records for conformance tests. Schema `1.46` exposes only their public consequences through `limitations`; it does not serialize a renderer support inventory.
