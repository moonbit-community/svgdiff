...
| SVG scene and canonical v1 raster | `Milky2018/svg@0.5.2` | Adopted for the current narrow support slice. |
...
The checked-in profile `/29` renderer-conformance baseline compares raw `Milky2018/svg@0.5.2` with Chromium 151 at DPR 1: 98 cases are exact and 43 diverge out of 141. It retains profile `/28` classifications and dispositions while recording exact `macos-arm64` and `linux-x64` measurement variants for the two pattern cases and one blend case whose Chromium rasters differ by host. The blend canonical fixture remains exact against the production W3C compositor and within a maximum per-channel delta of two against both browser hosts; the other 138 cases and every non-measurement field remain exact across them.

Every remaining divergence has an executable disposition. Profile `/29` deliberately retains the existing conservative renderer Diagnostics where one exact fixture does not prove the full transform, pattern, or stroke capability domain. The previously established `renderer_use_inherited_paint_raster_unproven` case still diverges. These guards limit only Rendered Evidence; exact source/computed differences and conservative localization remain available.

The typed engine model projects encountered renderer-specific Diagnostics into capability-gap records for conformance tests. Schema `5.0` exposes only their public consequences through `limitations`; it does not serialize a renderer support inventory.
