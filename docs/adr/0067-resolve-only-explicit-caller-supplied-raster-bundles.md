...
Comparisons are reproducible from closed caller-owned inputs and can distinguish different resources behind one locator without ambient state. Exact matching can reject keys that a browser URL resolver would consider equivalent; that is intentional because no base-URL contract exists. Bundle limits include unused entries because callers already supplied and allocated them, while signature and content validity remain lazy and are ignored when no SVG consumer selects the entry.

Module `0.5.4` adds the public bundle operations and CLI syntax. New missing, invalid-configuration, and resource-limit Diagnostic conditions advance Structured Report schema to `1.24`. Referenced content is validated lazily; unused payload bytes are not decoded or compared, while global bundle validation and byte budgets still apply. Renderer identity, conformance profile `/20`, intrinsic metric meaning, and ordering policy remain unchanged.
