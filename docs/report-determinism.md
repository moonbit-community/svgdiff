...
Status: current schema `1.32` contract
...
Rendered shape alignments are emitted first and retain their existing report-local IDs. Source-structural alignments follow in authored traversal order: authored-ID matches, structural-path matches, stable same-kind matches in sorted kind order, deletions, then insertions. The stable-order fallback is deterministic but remains explicitly tied and uncalibrated when several compatible endpoints exist.
