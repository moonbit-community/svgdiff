...
| How is compact-summary losslessness and graph traceability validated for M3? | [`m3-summary-traceability-gate/README.md`](../evaluation/m3-summary-traceability-gate/README.md) |
...
| How does M5 prove the adopted advanced-profile set and require pinned environments for future additions? | [`m5-adopted-profile-gate/README.md`](../evaluation/m5-adopted-profile-gate/README.md) |
...
- The stable report schema is `2.0`.
...
- Shared command specifications, configuration, and argument decoding live in `modules/svgdiff/cmd/cli`. The `modules/svgdiff/cmd/svgdiff` package exposes the same options, output modes, explicit resources, and optional checkpoint budget on native and WASIp1 through the standard async and filesystem APIs.
...
- The M5 adopted-profile gate derives an empty current advanced set and rejects adoption without consistent implementation, concrete profile, and pinned environment evidence.
...
- [`multi-renderer-profiles.md`](multi-renderer-profiles.md): future experiment cells, same-axis edge semantics, Agent synthesis, and permanent anti-majority/confounded-diagonal boundaries; explicitly no current container.
