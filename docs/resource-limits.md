...
Status: current module `0.3.1` and schema `1.3` contract
...
Path-data units deliberately form a conservative lexical work budget, not a segment count or a geometry metric. Complete path parsing and segment-level evidence remain separate roadmap work. Reference counting bounds reference-bearing input; cycle detection and adversarial reference expansion remain a separate Phase 11 item.

## Failure semantics

The engine checks raster dimensions before rendering, input bytes before parsing, XML structure while consuming one event stream, regions while extracting and attaching them, and report bytes at the serialization boundary. The exact inclusive boundary is accepted; the first unit beyond it is rejected.

An exceeded budget returns a small schema-valid report with:
...
Cancellation, wall-clock budgets, streaming input admission, peak-memory accounting, reference-cycle detection, and publicly configurable embedding policies remain explicit roadmap items. The generated HTML includes both bounded input sources in addition to report JSON and therefore has no separate report-byte identity.

## Executable evidence

[`resource_limits_wbtest.mbt`](../engine/resource_limits_wbtest.mbt) covers exact and one-past boundaries for every dimension, non-ASCII UTF-8 accounting, source locations, non-truncation, and bounded failure reports. [`test-cli.sh`](../scripts/test-cli.sh) covers the public failed-report and exit-status behavior.
