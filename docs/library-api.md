...
Status: current public interface for module version `0.10.0`

Last verified: 2026-07-20

Consumers should import the root package `Milky2018/svgdiff`. The `engine` package owns the concrete report types, while the root package deliberately re-exports them and pins the schema `5.0` comparison conditions.
...
`0.1.1` before root `0.10.0`; consumers still import only the root package.
...
The two canonical JSON methods serialize schema `5.0`. `to_json_string` uses indentation for inspection; `to_compact_json_string` removes only formatting whitespace and preserves every canonical field and value. `to_agent_projection_json_lines` emits the independent `svgdiff-agent-projection/2` JSONL transport: one header plus exact canonical section items that reconstruct the same report without loss.

Every call uses the fixed [comparison resource limits](resource-limits.md) and [local-reference admission guard](reference-safety.md). Crossing a source, structure, raster, region, or report budget returns a bounded `failed` report with `resource_limit_exceeded`; cyclic or explosively expanding accepted local references use their own stable Diagnostics. No failure returns a silently truncated difference inventory. The limits are intentionally not caller-configurable in module `0.10.0`.
...
| Subjects and alignment | `SubjectReference`, `SubjectInstanceContext`, `AlignmentEvidence`, `SubjectAlignment`, `VisualObjectSummary`, `VisualObjectBounds`, `VisualObjectAlignment`, `VisualRelationSummary` |
...
| Differences, grouping, and main-event assessment | `AtomicDifference`, `VisualEvent`, `VisualSceneAnalysis`, `VisualSceneSummary`, `VisualObjectChange`, `VisualSceneChange`, `VisualEvidenceCoverage`, `VisualDifferenceCoverage`, `VisualObjectChangeCoverage`, `VisualEvidenceDomain`, `ImpactAssessment`, `ImpactFrontierGroup`, `ImpactMeasurement`, `ImpactDominationWitness` |
...
The typed engine result still carries internal Impact Assessment state used by existing analysis and Markdown presentation. Schema `5.0` does not serialize its policy inputs, frontier groups, or domination witnesses. The formal JSON exposes the actual measurements and complete event inventory without presenting an internal selection mechanism as product evidence.
