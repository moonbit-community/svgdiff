...
| Parent transform-list, cumulative-matrix, and typed component change | Report the one authored transform cause, all referenced Atomic Differences, the typed component magnitude in its declared units, and the conservative changed scene regions | Turn raw matrix coefficients into a distance, compare unlike component units, or treat conservative whole-scene localization as precise transformed bounds |
