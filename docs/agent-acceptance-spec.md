...
| Parent transform-list and cumulative-matrix change | Report the one authored transform cause, both Atomic Differences, the changed scene regions, and the absence of a coefficient-distance score | Infer a rotation/scale decomposition that the report does not contain, or treat conservative whole-scene localization as precise transformed bounds |
