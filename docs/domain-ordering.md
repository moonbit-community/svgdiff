...
Schema `1.36` adds exact `parameter_abs_css_px`, `parameter_viewport_fraction`, and `parameter_entity_fraction` raw fields without adding them to v2 tuples. This preserves the accepted ordering identity: they are available for direct Agent explanation and future calibrated policy work, while existing geometry tuples continue to use outcome-oriented geometry and raster evidence.
...
5. For a question about the “main” change across domains, use an explicitly versioned Impact Assessment when one exists. Schema `1.36` has none, so report the strongest domain-specific evidence and disclose the interpretation instead of comparing tuple numbers.
