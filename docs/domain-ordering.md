...
5. For a question about the “main” change across domains, use an explicitly versioned Impact Assessment when one exists. Schema `1.25` has none, so report the strongest domain-specific evidence and disclose the interpretation instead of comparing tuple numbers.
