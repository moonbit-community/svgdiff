...
Status: current schema `1.40` serialization mode
...
The retained Comparison Profile includes `renderer_conformance_profile_id` and required nullable `perceptual_background`. A present background is normalized opaque sRGB8 policy input for `RenderedEvidence.perceptual_color`; it is not authored SVG paint or evidence that raw pixels were rendered over that color. Each perceptual-color object explicitly distinguishes computed evidence, absent background, and unavailable raw rendering. Consumers must not infer either profile identity from `schema_version` or `renderer_id`.
