...
Status: current schema `1.8` coverage map
...
| Deferred | The feature is intentionally outside schema `1.8`; current inputs are diagnosed through a partial guard. |
...
| Canonical unitless used geometry for supported basic shapes | Source facts and spans, computed relations, parameter magnitudes, alignment values, and conservative bounds | None for valid syntax; renderer guards remain independent | [`shape_geometry_wbtest.mbt`](../engine/shape_geometry_wbtest.mbt): numbers, defaults, radius propagation/clamping, point lists, zero geometry, and invalid values; [`generic_shape_diff_test.mbt`](../engine/generic_shape_diff_test.mbt): authored rewrites remain computed-equivalent |
...
| Root and nested SVG viewport mapping on the admitted integer axis slice | Strict `viewBox`, `preserveAspectRatio`, unitless, `px`, and percentage parsing; exact viewport facts and Source Spans; nearest-viewport percentage resolution; cumulative-matrix fan-out; typed effects; and conservative whole-scene raster outcome | None on the accepted integer axis slice | [`viewport_data_wbtest.mbt`](../engine/viewport_data_wbtest.mbt): grammar, alignments, meet/slice/none matrices, lengths, and percentages; [`viewport_diff_wbtest.mbt`](../engine/viewport_diff_wbtest.mbt): root intrinsic declarations, nested ordering, percentage basis, declaration facts, effects, and causes; browser oracle root and nested viewport fixtures |
...
| Invalid, non-positive, physical-unit, or otherwise unsupported viewport declaration | Partial | `viewport_semantics_unsupported` | Source, computed, rendered | [`viewport_diff_wbtest.mbt`](../engine/viewport_diff_wbtest.mbt): invalid declaration behavior; [`unsupported_input_property_test.mbt`](../engine/unsupported_input_property_test.mbt): zero dimensions, invalid `preserveAspectRatio`, and physical units cannot produce false complete equality |
...
The [deterministic mutation suite](../evaluation/mutations/README.md) declares and enforces the current six-kind, twenty-property coverage set independently of report output. Adding a kind or authored property to the supported fact inventory requires updating that contract in the same change.
