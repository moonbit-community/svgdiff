...
| JSON Schema | `1.39` | every serialized field, enum, null/absence rule, and top-level invariant |
...
Every released Schema, currently `1.0` through `1.39`, is a versioned consumer contract. A change to required fields, field meaning, enum values, null/absence behavior, identifier references, or numeric units requires an explicit compatibility review.
