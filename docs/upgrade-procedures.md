...
| --- | --- | --- |
| SVG scene and canonical renderer | `svgdiff/style-precedence-normalizer@2+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1` | `profile.renderer_id`, rendered magnitudes, Difference Regions, coverage guards |
| Renderer conformance profile | `svgdiff-renderer-conformance-profile/9` | conformance fixtures, dispositions, guards, thresholds, and Rendered Evidence claims |
...
| JSON Schema | `1.12` | every serialized field, enum, null/absence rule, and top-level invariant |
...
Every released Schema, currently `1.0` through `1.12`, is a versioned consumer contract. A change to required fields, field meaning, enum values, null/absence behavior, identifier references, or numeric units requires an explicit compatibility review.
