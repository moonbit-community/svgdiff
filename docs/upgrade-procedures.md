...
| JSON Schema | `1.31` | every serialized field, enum, null/absence rule, and top-level invariant |
...
Every released Schema, currently `1.0` through `1.31`, is a versioned consumer contract. A change to required fields, field meaning, enum values, null/absence behavior, identifier references, or numeric units requires an explicit compatibility review.
