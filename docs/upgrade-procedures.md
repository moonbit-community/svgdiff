...
| Renderer conformance profile | `svgdiff-renderer-conformance-profile/4` | conformance fixtures, dispositions, guards, thresholds, and Rendered Evidence claims |
...
| JSON Schema | `1.7` | every serialized field, enum, null/absence rule, and top-level invariant |
...
Every released Schema, currently `1.0` through `1.7`, is a versioned consumer contract. A change to required fields, field meaning, enum values, null/absence behavior, identifier references, or numeric units requires an explicit compatibility review.
