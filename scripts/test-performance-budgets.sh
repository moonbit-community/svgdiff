...
jq -e '
  ...
  .budget_version == "svgdiff-performance-budgets/2" and
  .passed == true and
  (.workloads | map(.size)) == ["small", "medium", "medium", "large"] and
  all(.workloads[]; .passed and (.samples | length) == 3 and all(.checks[]; .passed))
' "$tmp/results.json" >/dev/null

printf 'Performance budgets: small, medium, group-opacity, large time and peak RSS gates: ok\n'
