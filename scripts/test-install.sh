...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/5" and
...
grep -q '^svgdiff 0.4.2$' version.txt
grep -q '^schema: 1.8$' version.txt
grep -q '^renderer: svgdiff/style-precedence-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1$' version.txt
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/5$' version.txt
...
jq -e '.schema_version == "1.8" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/5" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.8" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.8" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
