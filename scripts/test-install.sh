...
grep -q '^svgdiff 0.5.15$' version.txt
grep -q '^schema: 1.35$' version.txt
...
jq -e '.schema_version == "1.35" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/25" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.35" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.35" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
