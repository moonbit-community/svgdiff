...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/22" and
...
grep -q '^svgdiff 0.5.7$' version.txt
grep -q '^schema: 1.27$' version.txt
...
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/22$' version.txt
...
jq -e '.schema_version == "1.27" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/22" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.27" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.27" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
