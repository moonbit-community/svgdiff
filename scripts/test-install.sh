...
grep -q '^svgdiff 0.5.13$' version.txt
grep -q '^schema: 1.33$' version.txt
...
jq -e '.schema_version == "1.33" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/25" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.33" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.33" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
