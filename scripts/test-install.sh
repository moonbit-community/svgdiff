...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/19" and
...
jq -e '.schema_version == "1.22" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/19" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.22" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.22" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
