...
grep -q '^svgdiff 0.3.2$' version.txt
...
jq -e '.schema_version == "1.4" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/1" and (.atomic_differences | length) == 1' agent.json >/dev/null
