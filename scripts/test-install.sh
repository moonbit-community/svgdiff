...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/9" and
...
grep -q '^svgdiff 0.4.6$' version.txt
grep -q '^schema: 1.12$' version.txt
grep -q '^renderer: svgdiff/style-precedence-normalizer@2+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1$' version.txt
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/9$' version.txt
...
jq -e '.schema_version == "1.12" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/9" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.12" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.12" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
