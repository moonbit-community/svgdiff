...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/14" and
...
grep -q '^svgdiff 0.4.11$' version.txt
grep -q '^schema: 1.17$' version.txt
...
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/14$' version.txt
...
jq -e '.schema_version == "1.17" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/14" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.17" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.17" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
