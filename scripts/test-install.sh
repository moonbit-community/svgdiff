...
jq -e '.schema_version == "5.0" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "5.0" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
