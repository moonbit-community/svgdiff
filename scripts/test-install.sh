...
grep -q '^svgdiff 0.7.1$' version.txt
grep -q '^schema: 2.0$' version.txt
...
jq -e '.schema_version == "2.0" and (.difference_groups | map(.items | length) | add) == 1' agent.json >/dev/null
...
test ! -s summary.err
jq -e '.schema_version == "2.0"' summary-report.json >/dev/null
...
jq -e '.schema_version == "2.0" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "2.0" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
