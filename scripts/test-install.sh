...
jq -e '
  ...
  .profile.renderer_conformance_profile_id ==
    "svgdiff-renderer-conformance-profile/11" and
...
grep -q '^svgdiff 0.4.8$' version.txt
grep -q '^schema: 1.14$' version.txt
grep -q '^renderer: svgdiff/style-precedence-normalizer@3+ordinary-inheritance-normalizer@1+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1$' version.txt
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/11$' version.txt
...
jq -e '.schema_version == "1.14" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/11" and (.atomic_differences | length) == 1' agent.json >/dev/null
...
jq -e '.schema_version == "1.14" and .analysis_status == "complete"' stdin-report.json >/dev/null
...
jq -e '.schema_version == "1.14" and .analysis_status == "complete"' stdin-after-report.json >/dev/null
