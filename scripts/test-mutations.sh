...
jq -e '
  ...
  (.coverage_contract.source_properties | sort) == ["cx", "cy", "fill", "height", "opacity", "points", "r", "rx", "ry", "stroke", "stroke-width", "transform", "viewBox", "width", "x", "x1", "x2", "y", "y1", "y2"] and
...
printf 'Mutation cases: %s, subject kinds: 6, source properties: 20, deterministic generation: ok, changed facts: ok\n' "$(jq '.cases | length' "$manifest")"
