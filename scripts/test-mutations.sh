...
jq -e '
  ...
  (.coverage_contract.source_properties | sort) == ["--paint", "color", "cx", "cy", "fill", "fill-opacity", "height", "marker-end", "markerUnits", "markerWidth", "opacity", "orient", "points", "r", "refX", "rx", "ry", "stroke", "stroke-dasharray", "stroke-dashoffset", "stroke-linecap", "stroke-linejoin", "stroke-miterlimit", "stroke-opacity", "stroke-width", "transform", "vector-effect", "viewBox", "width", "x", "x1", "x2", "y", "y1", "y2"] and
...
printf 'Mutation cases: %s, subject kinds: 6, source properties: 35, deterministic generation: ok, changed facts: ok\n' "$(jq '.cases | length' "$manifest")"
