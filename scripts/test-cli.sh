...
jq -e '.schema_version == "1.14" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/11" and .analysis_status == "complete" and (.coverage_matrix | length) > 0 and .renderer_capability_gaps == [] and (all(.coverage_matrix[]; (.source_semantics != "limited" and .computed_appearance != "limited" and .rendered_evidence != "limited"))) and (.atomic_differences | length) == 1' "$tmp/report.json" >/dev/null
...
jq -e '.schema_version == "1.14" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/11" and .analysis_status == "complete" and (.atomic_differences | length) == 1' "$tmp/agent.json" >/dev/null
...
jq -e '.profile.viewport_width == 32 and .profile.viewport_height == 24 and .profile.comparison_dpr == 1 and .profile.color_interpretation == "srgb" and .profile.raster_representation == "linear_srgb_premultiplied_rgba_f64" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/11" and (.events[0].rendered_outcome.magnitude.linear_premultiplied_rgba_rmse > 0)' "$tmp/output.json" >/dev/null
...
jq -e '.schema_version == "1.14" and .analysis_status == "complete"' "$tmp/stdin-before.json" >/dev/null
...
jq -e '.schema_version == "1.14" and .analysis_status == "complete"' "$tmp/stdin-after.json" >/dev/null
...
grep -q '^svgdiff 0.4.8$' "$tmp/version.txt"
grep -q '^engine: 0.4.8$' "$tmp/version.txt"
grep -q '^schema: 1.14$' "$tmp/version.txt"
grep -q '^renderer: svgdiff/style-precedence-normalizer@3+ordinary-inheritance-normalizer@1+length-used-value-normalizer@1+stroke-used-geometry-normalizer@1+basic-shape-used-geometry-normalizer@1+mizchi/svg@0.2.1$' "$tmp/version.txt"
grep -q '^renderer-conformance-profile: svgdiff-renderer-conformance-profile/11$' "$tmp/version.txt"
