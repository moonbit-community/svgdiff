...
jq -e '.schema_version == "1.36" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/25" and .analysis_status == "complete" and (.coverage_matrix | length) > 0 and .renderer_capability_gaps == [] and (all(.coverage_matrix[]; (.source_semantics != "limited" and .computed_appearance != "limited" and .rendered_evidence != "limited"))) and (.atomic_differences | length) == 1' "$tmp/report.json" >/dev/null
...
jq -e '.schema_version == "1.36" and .profile.renderer_conformance_profile_id == "svgdiff-renderer-conformance-profile/25" and .analysis_status == "complete" and (.atomic_differences | length) == 1' "$tmp/agent.json" >/dev/null
...
jq -e '.schema_version == "1.36" and .analysis_status == "complete"' "$tmp/stdin-before.json" >/dev/null
...
jq -e '.schema_version == "1.36" and .analysis_status == "complete"' "$tmp/stdin-after.json" >/dev/null
...
grep -q '^svgdiff 0.5.16$' "$tmp/version.txt"
grep -q '^engine: 0.5.16$' "$tmp/version.txt"
grep -q '^schema: 1.36$' "$tmp/version.txt"
