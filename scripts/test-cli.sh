...
cd "$root"
moon run --target native cmd/svgdiff -- testdata/before.svg testdata/after.svg >"$tmp/report.json"
jq -e '.schema_version == "1.0" and .analysis_status == "complete" and (.atomic_differences | length) == 1' "$tmp/report.json" >/dev/null

moon run --target native cmd/svgdiff -- testdata/before.svg testdata/after.svg --width 32 --height 24 --output "$tmp/output.json" --html "$tmp/report.html"
...
if moon run --target native cmd/svgdiff -- "$tmp/missing.svg" testdata/after.svg >/dev/null 2>&1; then
  echo "CLI unexpectedly accepted an unreadable input" >&2
  exit 1
fi

printf '%s\n' '<svg><rect></svg>' >"$tmp/malformed.svg"
if moon run --target native cmd/svgdiff -- "$tmp/malformed.svg" testdata/after.svg >"$tmp/failed.json"; then
